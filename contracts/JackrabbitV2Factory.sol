...
contract JackrabbitV2Factory is IJackrabbitV2Factory {
    ...
    function pairCodeHash() external pure returns (bytes32) {
        return keccak256(type(JackrabbitV2Pair).creationCode);
    }

    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        require(tokenA != tokenB, 'JackrabbitV2: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'JackrabbitV2: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'JackrabbitV2: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(JackrabbitV2Pair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        JackrabbitV2Pair(pair).initialize(token0, token1);
